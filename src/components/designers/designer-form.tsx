...
type DesignerFormProps = {
  designer?: Designer;
  theme?: "light" | "dark";
  /** В модалке — без внешней карточки и заголовка (заголовок в шапке модалки). */
  variant?: "card" | "modal";
  onCancel: () => void;
  onSaved?: () => void;
};
...
export function DesignerForm({
  ...
  return (
    ...
    >
      ...
      {error ? (
        <p className={`text-sm ${errorCls} ${!isModal ? "mt-3" : "mb-1"}`}>
          {error}
        </p>
      ) : null}

      <div className={`space-y-4 ${!isModal ? "mt-4" : ""}`}>
      ...
      <div className={`flex gap-2 ${isModal ? "pt-4" : "mt-6"}`}>
